...
function newspro_tree_output_cat($tree,$type) {
  ...
  $num_items = count($items);
  $s = ''; //if ($data['link']['language'] == $GLOBALS['language']->language)
  foreach ($items as $i => $data) if (!isset($data['link']['language']) or $data['link']['language'] == $GLOBALS['language']->language or $data['link']['language'] == 'und') {
    //drupal_set_message('<pre>'. check_plain(print_r($data, 1)) .'</pre>');
	  if ($data['link']['in_active_trail']) $a = ' class="active"'; else $a = '';
    if ($data['below']) {
